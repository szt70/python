...
class Doc2Vec(Word2Vec):
    ...
    def most_similar_words(self, positive=[], negative=[], topn=10):
        ...
        result = [(k, v) for (k, v) in result if k not in self.tags]
        return result[:topn]

    #ADD
    def build_tags(self, sentences):
        self.tags |= self._tags_from(sentences)
    
    #ADD
    @staticmethod
    def _tags_from(sentences):
        tags = set()
        for sentence in sentences:
            tags |= set(sentence.tags)
        return tags

    #ADD
    def build_vocab(self,sentences):
        self.build_tags(sentences)
        super(Doc2Vec, self).build_vocab(sentences)
